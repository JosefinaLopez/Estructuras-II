...
namespace GrafoConMatrizDeAdyacencia
{
    class GrafoDirigido6
    {
        ...
        public static void Fuente()
        {
            int x = 0;
            bool band = false;
            Pedir("De el Nodo o Vertice: ", ref x);
            for (int i = 0; i < V; i++)
            {
                for (int j = 0; j < V; j++)
                {
                    if (matrizAdyacencia[i, j])
                    {  
                        //!Si el nodo esta contenido en alguna de las lista de demas nodos se cambia a true
                        if (x == j)
                        { 
            ...
            //!Imprime el caso de que el nodo dado no este contenido en ninguna de la listas , ni en la propia
            if(!band)
            {
                System.Console.WriteLine($"El Nodo o Vertice {x} es Fuente");
            }
        ...
        public static void CrearGrafo()
        {
            ...
            GrafoDirigido6 grafo = new GrafoDirigido6(numVertices);
