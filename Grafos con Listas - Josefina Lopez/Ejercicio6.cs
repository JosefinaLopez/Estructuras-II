...
namespace Grafos6
{
    class GrafoDirigido
    {
        ...
        public static bool Fuente()
        {
            bool band = false;
            int x = 0;
            Pedir("Escriba el Nodo:", ref x);
            for (int i = 0; i < V; i++)
            {
                foreach (var nodos in listaAdyacencia[i])
                {   
                    //!Verifico que el vertice no este contenido en la lista de los demas vertices
                    if(x != nodos && !band)
                    {   
                        return band = true;
                    }
            ...
            do
            {
                ...
                switch (op)
                {
                    ...
                    case 2:
                        if(Fuente())
                        {
                            System.Console.WriteLine("Es un nodo fuente");
                        }
                        else{
                            System.Console.WriteLine("No es un nodo fuente");
                        }
                        break;
                    case 3:
                        ImprimirGrafo();
